...
impl Solution {
    pub fn get_skyline(buildings: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
        ...
        if !buildings.iter().all(|b| b.len() == 3) {
            unreachable!();
        }
        let cuts: BTreeSet<i32> = buildings
            .iter()
            .flat_map(|b| (&b[..2]).iter())
            .copied()
            .collect();
        let mut pq: BinaryHeap<(i64, i64)> = BinaryHeap::new();
        pq.push((0_i64, i64::max_value()));
        let mut i: usize = 0;
        let mut out: Vec<Vec<i32>> = vec![vec![0; 2]];
        for cut in cuts {
            while i < buildings.len() && buildings[i][0] <= cut {
                pq.push((buildings[i][2] as i64, buildings[i][1] as i64));
                i += 1;
            }
            while let Some(&(_, value)) = pq.peek() {
                if value <= cut as i64 {
                    pq.pop();
                } else {
                    break;
            ...
            if let Some(&(high, _)) = pq.peek() {
                if out[out.len() - 1][1] as i64 != high {
                    out.push(vec![cut, high as i32]);
                }
            } else {
                unreachable!();
            }
